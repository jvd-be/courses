...
export default function Latest() {
  ...
  useEffect(() => {
    setLoad(false)
    fetch("http://localhost:5000/courses?_sort=id&_order=desc&_limit=8")
      .then((res) => res.json())
      .then((data) => {
        setLastCourse(data)
        setLoad(true)
  ...
  return (
    <div className="px-16 pt-6 pb-6 ">
      <div className="section_last text-slate-500 text-base font-sf my-8 dark:text-yellow-50">جدیدترین دوره ها</div>
      {load?(<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 ">
        {lastCourse.map(
          ({
            id,
            title,
                ...
                <div className="flex items-end justify-between mt-4">
                  ...
                  <div className="flex justify-evenly items-center w-full">
                      ...
                      {price} میلیون تومان 
                          ...
                          <a  onClick={()=>handleNavigate(id)} className="text-white cursor-pointer  bg-sky-600 hover:bg-sky-700 px-4 py-2 rounded-md font-sf text-base ">ثبت نام</a>
